...
var log_dir = process.argv.length > 2 ? process.argv[2] :  "/dev/snd/";
var logs = [];
...
var tail = spawn("cat",  ["/dev/snd/midiC0D0"]);
...
io.sockets.on('connection', function(client){
  var filename;
	tail.stdout.setEncoding('base64');
	tail.stdout.on("data", function (data) {
		console.log(data.toString(16));
		if(data.length > 5) {
			client.volatile.emit('message', data.charAt(8) + data.charAt(9));
